...
void SMS_Yields() {
	...
	TString fileName = "200317/DiTauMC/SMS_TChipmStauSnu/H2TauTauTreeProducerTauTau/tree.root";

	double lumi = 35900.;
	double X_sec = 387.534; // 58.6311;
	double nGenEvts = 47585; // 45607.0;
	...
	double xsec_weight = lumi * X_sec / (nGenEvts * 1000.);
	...
	if (!(tr.mvis>100 && tr.n_bjets==0 && tr.mt + tr.mt_leg2 > 150.)) continue;
        ...
        if (!(tr.GenSusyMChargino == 250 && tr.GenSusyMNeutralino == 50)) continue;
